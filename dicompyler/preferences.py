...
class PreferencesManager():
    ...
    def SetPreferenceValue(self, msg):
        ...
        pub.sendMessage(list(msg.keys())[0],
...
class PreferencesDialog(wx.Dialog):
    ...
    def OnUpdateChoice(self, evt):
        ...
        pub.sendMessage(self.callbackdict[c],
                        topic=self.callbackdict[c],
                        msg=evt.GetString())
    ...
    def OnUpdateSlider(self, evt):
        ...
        pub.sendMessage(self.callbackdict[s],
                        topic=self.callbackdict[s],
                        msg=s.GetValue())
    ...
    def OnUpdateDirectory(self, evt):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            d = str(dlg.GetPath())
            t.SetValue(d)
            pub.sendMessage(self.callbackdict[b],
                            topic=self.callbackdict[b],
                            msg=d)
...
def main():
    ...
    filename = t.name
    frame.prefmgr = PreferencesManager(parent=frame,
                                       appname='preftest',
                                       filename=filename)
    ...
    grp1template = [{
        'Panel 1 Preference Group 1': [{
            ...
            'Choice 2',
            'callback':
            'panel1.prefgrp1.choice_setting'
        }, {
            'name':
            'Directory setting',
            'type':
            'directory',
            'default':
            str(sp.GetDocumentsDir()),
            'callback':
            'panel1.prefgrp1.directory_setting'
        }]
    }, {
    ...
    grp2template = [{
        ...
        'Panel 2 Preference Group 2': [{
            ...
            'directory',
            'default':
            str(sp.GetUserDataDir()),
            'callback':
            'panel2.prefgrp2.directory_setting'
        }, {
            'name':
            'Choice Setting',
            'type':
            ...
            'Choice 2',
            'callback':
            'panel2.prefgrp2.choice_setting'
        }]
    }]
